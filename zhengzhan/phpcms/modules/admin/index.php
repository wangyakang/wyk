...
class index extends admin {
		...
		if(isset($_GET['dosubmit'])) {
			...
			if (!isset($_GET['card'])) {
				...
				if (!isset($_SESSION['card_verif']) || $_SESSION['card_verif'] != 1) {
					showmessage(L('your_password_card_is_not_validate'), '?m=admin&c=libs&a=public_card');
				}
			...
			if(!$r) showmessage(L('user_not_exist'),'?m=admin&c=libs&a=login');
			...
			if($r['password'] != $password) {
				...
				showmessage(L('password_error',array('times'=>$times)),'?m=admin&c=libs&a=login',3000);
			}
			...
			if (!isset($_GET['card']) && $r['card'] && pc_base::load_config('system', 'safe_card') == 1) {
				...
				header("location:?m=admin&c=libs&a=public_card");
				exit;
			...
			showmessage(L('login_success'),'?m=admin&c=libs');
			//同步登陆vms,先检查是否启用了vms
	...
	public function public_card() {
		...
		if (isset($_GET['dosubmit'])) {
			...
			header("location:?m=admin&c=libs&a=login&dosubmit=1&card=1");
			exit;
		}
		pc_base::load_app_class('card', 'admin', 0);
	...
	public function public_logout() {
		...
		showmessage(L('logout_success').$phpsso_logout,'?m=admin&c=libs&a=login');
	}
