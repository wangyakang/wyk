...
class html {
	...
	public function _index($specialid = 0, $pagesize = 20, $pages_num = 0) {
		...
		if ($siteid>1) {
			...
			$file = pc_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/'.$filename.'/libs.html';
		} else {
			$file = pc_base::load_config('system', 'html_root').'/special/'.$filename.'/libs.html';
		}
		if(!$ispage) {
		...
		$template = $index_template ? $index_template : 'libs';
		...
		if($ispage) {
			...
			for ($i=1; $i<=$pages_num; $i++) {
				...
				else $file_root = str_replace('libs', 'libs-'.$i, $file);
	...
	public function create_list($page = 1) {
		$siteid = get_siteid();
		$site_info = $this->site->get_by_id($siteid);
		define('URLRULE', $site_info['domain'].substr(pc_base::load_config('system', 'html_root'), 1).'/special/libs.html~'.$site_info['domain'].substr(pc_base::load_config('system', 'html_root'), 1).'/special/libs-{$page}.html');
		//分站时计算路径
		if ($siteid>1) {
			if ($page==1) $file = pc_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/libs.html';
			else $file = pc_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/libs-'.$page.'.html';
		} else {
			if ($page==1)  $file = pc_base::load_config('system', 'html_root').'/special/libs.html';
			else $file = pc_base::load_config('system', 'html_root').'/special/libs-'.$page.'.html';
		}
		$this->queue->add_queue('add', $file, $siteid);
		$file  = PHPCMS_PATH.$file;
		ob_start();
