 ...
 * @return unknown_type
 */
function template($module = 'content', $template = 'libs', $style = '') {
	...
	if($datas['style']=='1') {
		...
		$string .= '<script type="text/javascript">
					$(function(){
						...
						function onchange(e){
							var $target = $(e.target);
							var libs = $ld5.libs($target);
							$("#'.$id.'-'.$i.'").remove();
							$("#'.$id.'").val($ld5.eq(libs).show().val());
							libs ++;
							$ld5.eq(libs).show();								}
					})
		</script>';
 ...
 */
function p_template($plugin = 'content', $template = 'libs',$style='default') {
	if(!$style) $style = 'default';
	$template_cache = pc_base::load_sys_class('template_cache');
