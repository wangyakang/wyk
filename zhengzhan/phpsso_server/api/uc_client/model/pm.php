...
/*
	function deletepmbypmids($uid, $pmids) {
		...
		while($libs = $this->db->fetch_array($query)) {
			$users = explode('_', $libs['min_max']);
			if(!in_array($uid, $users)) {
				return PMPRIVILEGENONE_ERROR;
			}
			if($libs['authorid'] == $uid) {
				$delstatus1pm[$this->getposttablename($libs['plid'])][] = $libs['pmid'];
			} else {
				$delstatus2pm[$this->getposttablename($libs['plid'])][] = $libs['pmid'];
			}
			$pmplidpost[$this->getposttablename($libs['plid'])][] = $libs['plid'];
			$pmplid[$libs['plid']] = $libs['plid'];
		}
		if(empty($pmplidpost)) {
			return 0;
		}
