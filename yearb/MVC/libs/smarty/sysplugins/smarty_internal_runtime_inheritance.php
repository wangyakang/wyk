...
class Smarty_Internal_Runtime_Inheritance
{
     ...
     * - 3 assume parent template, if child will loaded goto status 1
     *     a call to a sub template resets the status to 0
     *
     * @var int
     */
    public $state = 0;
    ...
    public function init(Smarty_Internal_Template $tpl, $initChild, $blockNames = array())
    {
        ...
        // if status was waiting for parent change status to parent
        if ($this->state == 2) {
            $this->state = 3;
        }
    }
     ...
     * - if outer level is reached flush output buffer and switch to wait for parent template status
     *
     */
     ...
     * - if outer level {block} of child template ($status == 1) save it as child root block
