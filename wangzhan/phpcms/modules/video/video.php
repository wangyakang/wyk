...
class video extends admin {
	...
	public function add() {
		if ($_POST['dosubmit']) {
			...
			if (!$data['vid']) showmessage(L('failed_you_video_uploading'), 'libs.php?m=video&c=video&a=add');
			$data['title'] = isset($_POST['title']) && trim($_POST['title']) ? trim($_POST['title']) : showmessage(L('video_title_not_empty'), 'libs.php?m=video&c=video&a=add&meunid='.$_GET['meunid']);
			...
			if ($videoid) {
				showmessage(L('operation_success'), 'libs.php?m=video&c=video&a=init&meunid='.$_GET['meunid']);
			} else {
				showmessage(L('operation_failure'), 'libs.php?m=video&c=video&a=add&meunid='.$_GET['meunid']);
			}
		} else {
			if((empty($this->setting['sn']) || empty($this->setting['skey'])) && ROUTE_A!='open') {
				header("Location: ".APP_PATH."libs.php?m=video&c=video&a=open&meunid=".$_GET['meunid'].'&pc_hash='.$_GET['pc_hash']);
			}
	...
	public function edit() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			if ($this->ku6api->vms_edit($data)) {
				...
				if ($return) showmessage(L('operation_success'), 'libs.php?m=video&c=video&a=init');
				else showmessage(L('operation_failure'), 'libs.php?m=video&c=video&a=edit&vid='.$vid.'&menuid='.$_GET['menuid']);
			} else {
				showmessage($this->ku6api->error_msg, 'libs.php?m=video&c=video&a=edit&vid='.$vid.'&menuid='.$_GET['menuid']);
			}
		} else {
			$show_validator = true;
	...
	public function delete() {
		...
		if (!$this->ku6api->delete_v($r['vid'])) showmessage(L('operation_failure'), 'libs.php?m=video&c=video&a=init&meunid='.$_GET['meunid']);
		$this->v->del_video($vid);	
		showmessage(L('success_next_update_content'), 'libs.php?m=video&c=video&a=public_update_content&vid='.$vid.'&meunid='.$_GET['meunid']);
	}
	...
	public function public_update_content() {
		...
		if (!$result || empty($result)) {
			showmessage(L('update_complete'), 'libs.php?m=video&c=video&a=init&meunid='.$meunid);
		}
		//加载更新html类
		...
		showmessage(L('part_update_complete'), 'libs.php?m=video&c=video&a=public_update_content&vid='.$videoid.'&meunid='.$meunid);
	}
	...
	public function subscribe_list() {
		if (isset($_GET['dosubmit'])) {
			if (is_array($_GET['sub']) && !empty($_GET['sub'])) {
				...
				if ($result['code'] == 200) showmessage(L('operation_success'), 'libs.php?m=video&c=video&a=subscribe_list');
				else showmessage(L('subscribe_set_failed'), 'libs.php?m=video&c=video&a=subscribe_list&meunid='.$_GET['meunid']);
			} else {
				showmessage(L('please_choose_catid_and_channel'), 'libs.php?m=video&c=video&a=subscribe_list&meunid='.$_GET['meunid']);
			}
		} else {
	...
	public function sub_del() {
		$id = intval($_GET['id']);
		if (!$id) showmessage(L('illegal_parameters'), 'libs.php?m=video&c=video&a=subscribe_list&meunid='.$_GET['meunid']);
		if ($this->ku6api->sub_del($id)) showmessage(L('operation_success'), 'libs.php?m=video&c=video&a=subscribe_list&meunid='.$_GET['meunid']);
		else showmessage(L('delete_failed'), 'libs.php?m=video&c=video&a=subscribe_list&meunid='.$_GET['meunid']);
	}
	...
	public function user_sub_del() {
		...
		if (!$id) showmessage(L('illegal_parameters'), 'libs.php?m=video&c=video&a=subscribe_list&meunid='.$_GET['meunid']);
		if ($this->ku6api->user_sub_del($id)) showmessage(L('operation_success'), 'libs.php?m=video&c=video&a=subscribe_list&meunid='.$_GET['meunid'].'&type='.$type);
		else showmessage(L('delete_failed'), 'libs.php?m=video&c=video&a=subscribe_list&meunid='.$_GET['meunid'].'&type='.$type);
	}	
	...
	*/
		...
		$this->setting = getcache('video');
 		if(empty($this->setting['skey']) || empty($this->setting['sn'])){
			...
			if ($return_check==200 && SITE_URL != 'localhost' && !preg_match("/^(127|192|10)\.([1-2]?)([0-9]?)([0-9])\.([1-2]?)([0-9]?)([0-9])\.([1-2]?)([0-9]?)([0-9])/", SITE_URL)) {//存在同域名记录，进行email验证
				header("Location: ".APP_PATH."libs.php?m=video&c=video&a=check_user_back&meunid=".$_GET['meunid'].'&pc_hash='.$_GET['pc_hash']);
				exit;
			}
	...
	public function subscribe_uservideo() {
		if (is_array($_GET['sub']) && !empty($_GET['sub'])) {
			...
			if ($result['check'] == 6) showmessage(L('subscribe_for_default')); 
			if ($result['code'] == 200) showmessage(L('operation_success'), 'libs.php?m=video&c=video&a=subscribe_list&type=2');
			else showmessage(L('subscribe_set_failed'), 'libs.php?m=video&c=video&a=subscribe_list&meunid='.$_GET['meunid']);
		} else {
			showmessage(L('please_choose_catid_and_iputuserid'), 'libs.php?m=video&c=video&a=subscribe_list&meunid='.$_GET['meunid'].'&type=2');
		}
	}	
