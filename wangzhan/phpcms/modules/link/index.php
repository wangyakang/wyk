...
class index {
	...
	public function register() { 
 		...
 		if(isset($_POST['dosubmit'])){
 			if($_POST['name']==""){
 				showmessage(L('sitename_noempty'),"?m=link&c=libs&a=register&siteid=$siteid");
 			}
 			if($_POST['url']=="" || !preg_match('/^http:\/\/(.*)/i', $_POST['url'])){
 				showmessage(L('siteurl_not_empty'),"?m=link&c=libs&a=register&siteid=$siteid");
 			}
			...
			$url = trim_script($url);
 			...
 			$link_db->insert($sql);
 			showmessage(L('add_success'), "?m=link&c=libs&siteid=$siteid");
 		} else {
  			$setting = getcache('link', 'commons');
