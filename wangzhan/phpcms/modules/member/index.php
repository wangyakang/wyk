...
class index extends foreground {
	...
	public function register() {
		...
		if(!$member_setting['allowregister']) {
			showmessage(L('deny_register'), 'libs.php?m=member&c=libs&a=login');
		}
		//加载短信模块配置
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($member_setting['enablemailcheck']) {	//是否需要邮件验证
				...
				showmessage(L('operation_success'), 'libs.php?m=member&c=libs&a=register&t=3');
			} else {
				//查看当前模型是否开启了短信验证功能
			...
			if(pc_base::load_config('system', 'phpsso')) {
				...
				if($status > 0) {
					...
					if($member_setting['enablemailcheck']) {
						...
						$url = APP_PATH."libs.php?m=member&c=libs&a=register&code=$code&verify=1";
						...
						showmessage(L('operation_success'), 'libs.php?m=member&c=libs&a=register&t=2');
					} else {
						//如果不需要邮箱认证、直接登录其他应用
						$synloginstr = $this->client->ps_member_synlogin($userinfo['phpssouid']);
						showmessage(L('operation_success').$synloginstr, 'libs.php?m=member&c=libs&a=init');
					}
					
				}
			} else {
				showmessage(L('enable_register').L('enable_phpsso'), 'libs.php?m=member&c=libs&a=login');
			}
			...
			if(!empty($_GET['verify'])) {
				$code = isset($_GET['code']) ? trim($_GET['code']) : showmessage(L('operation_failure'), 'libs.php?m=member&c=libs');
				$code_res = sys_auth($code, 'DECODE', get_auth_key('email'));
				$code_arr = explode('|', $code_res);
				$userid = isset($code_arr[0]) ? $code_arr[0] : '';
				$userid = is_numeric($userid) ? $userid : showmessage(L('operation_failure'), 'libs.php?m=member&c=libs');

				$this->db->update(array('groupid'=>$this->_get_usergroup_bypoint()), array('userid'=>$userid));
				showmessage(L('operation_success'), 'libs.php?m=member&c=libs');
	...
	public function account_manage_avatar() {
		...
		$upurl = base64_encode($phpsso_api_url.'/libs.php?m=phpsso&c=libs&a=uploadavatar&auth_data='.$auth_data);
		//获取头像数组
		$avatar = $this->client->ps_getavatar($this->memberinfo['phpssouid']);
	...
	public function account_manage_upgrade() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($memberinfo['amount'] >= $cost) {
				...
				showmessage(L('operation_success'), 'libs.php?m=member&c=libs&a=init');
			} else {
				showmessage(L('operation_failure'), HTTP_REFERER);
			}
		...
		if(isset($_POST['dosubmit'])) {
			...
			if(pc_base::load_config('system', 'phpsso')) {
				...
				if(isset($memberinfo['uid'])) {
					...
					if($status == -1) {	//用户不存在
						showmessage(L('user_not_exist'), 'libs.php?m=member&c=libs&a=login');
					} elseif($status == -2) { //密码错误
						showmessage(L('password_error'), 'libs.php?m=member&c=libs&a=login');
					} else {
						showmessage(L('login_failure'), 'libs.php?m=member&c=libs&a=login');
					}
				}
				...
				if(!$r) showmessage(L('user_not_exist'),'libs.php?m=member&c=libs&a=login');
				...
				if($r['password'] != $password) {				
					...
					showmessage(L('password_error', array('times'=>$times)), 'libs.php?m=member&c=libs&a=login', 3000);
				}
			...
			param::set_cookie('_nickname', $nickname, $cookietime);
			//param::set_cookie('cookietime', $_cookietime, $cookietime);
			$forward = isset($_POST['forward']) && !empty($_POST['forward']) ? urldecode($_POST['forward']) : 'libs.php?m=member&c=libs';
	...
	public function logout() {
		...
		if($setting['snda_enable'] && param::get_cookie('_from')=='snda') {
			...
			$forward = isset($_GET['forward']) && trim($_GET['forward']) ? urlencode($_GET['forward']) : '';
			$logouturl = 'https://cas.sdo.com/cas/logout?url='.urlencode(APP_PATH.'libs.php?m=member&c=libs&a=logout&forward='.$forward);
			...
			$forward = isset($_GET['forward']) && trim($_GET['forward']) ? $_GET['forward'] : 'libs.php?m=member&c=libs&a=login';
	...
	public function public_sina_login() {
		...
		define('WEB_CALLBACK', APP_PATH.'libs.php?m=member&c=libs&a=public_sina_login&callback=1');
		...
		if(isset($_GET['callback']) && trim($_GET['callback'])) {
			...
			if(!empty($me['id'])) {
				...
				if(!empty($r)) {
					...
					param::set_cookie('cookietime', $_cookietime, $cookietime);
					param::set_cookie('_nickname', $nickname, $cookietime);
					$forward = isset($_GET['forward']) && !empty($_GET['forward']) ? $_GET['forward'] : 'libs.php?m=member&c=libs';
					showmessage(L('login_success').$synloginstr, $forward);
	...
	public function public_snda_login() {
		...
		define('SNDA_CALLBACK', urlencode(APP_PATH.'libs.php?m=member&c=libs&a=public_snda_login&callback=1'));
	...
	public function public_qq_login() {
		...
		if(isset($_GET['callback']) && trim($_GET['callback'])) {
			...
			$oauth_callback = APP_PATH.'libs.php?m=member&c=libs&a=public_qq_login&callback=1';
	...
	public function public_forget_password () {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$url = APP_PATH."libs.php?m=member&c=libs&a=public_forget_password&code=$code";
			...
			showmessage(L('operation_success'), 'libs.php?m=member&c=libs&a=login');
		} elseif($_GET['code']) {
			...
			if(is_array($code) && is_numeric($code[0]) && date('y-m-d h', SYS_TIME) == date('y-m-d h', $code[1])) {
				...
				if(empty($memberinfo['phpssouid'])) {
					showmessage(L('operation_failure'), 'libs.php?m=member&c=libs&a=login');
				}
				$updateinfo = array();
