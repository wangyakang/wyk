...
class special extends admin {
			...
			if ($id) {
				...
				if ($special['siteid']>1) {
					...
					if ($special['ishtml']) {
						...
						$url = $site_info['domain'].'libs.php?m=special&c=libs&id='.$id;
					}
				} else {
					$url = $special['ishtml'] ? APP_PATH.substr(pc_base::load_config('system', 'html_root'), 1).'/special/'.$special['filename'].'/' : APP_PATH.'libs.php?m=special&c=libs&id='.$id;
				}
	...
	public function edit() {
		...
		if (isset($_POST['dosubmit']) && !empty($_POST['dosubmit'])) {
			...
			if ($special['ishtml'] && $special['filename']) {
				...
				if ($siteid>1) {
					$special['url'] = $site_info['domain'].'libs.php?m=special&c=libs&specialid='.$_GET['specialid'];
				} else {
					$special['url'] = APP_PATH.'libs.php?m=special&c=libs&specialid='.$_GET['specialid'];
				}
			}
	...
	private function check($data, $a = 'add') {
		...
		if($a=='add') {
			if(!$data['index_template']) $data['index_template'] = 'libs';
